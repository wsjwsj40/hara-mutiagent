...
5. 先读取 `references/json-contracts.md` 顶部的“完整输出 Schema（严格遵循）”。
6. 严格按 Schema 构建 `meta`、`mf_vehicle_hazards`、`hazard_reasoning`、`knowledge_evidence`、`review_log`；字段名、顶层 key、数组/对象结构不要改名或增包一层。
7. 只写 UTF-8 JSON，不输出 Markdown 包裹。
8. 返回前运行验证并修正结构/数量问题；验证失败时修 JSON 文件，不用解释代替修复。
