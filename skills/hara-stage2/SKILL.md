---
name: hara-stage2
description: Stage 2 整车危害生成。用于将 Stage 1 单功能片段中的每个非 nan 功能故障转换为整车级危害，按 Function_ID 生成 output/<RUN_ID>_stage2_<Function_ID>_mf_vehicle_hazards.json。最终 Stage2 合并必须在 Stage2R 单片语义评审通过后执行。不要用于场景生成、SEC 评级、ASIL 计算或安全目标。
---
...
1. 枚举当前 Stage1 单功能片段中的非 `nan` 故障字段。
2. 每个非 `nan` 故障生成一条 `mf_vehicle_hazards`。
