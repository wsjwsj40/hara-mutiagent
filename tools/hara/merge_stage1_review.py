#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Merge Stage 1R single-function review JSON files into one review JSON."""
...
def infer_run_id(prefix: str | None, paths: list[Path]) -> str:
    if prefix:
        return prefix
    for path in paths:
        match = re.match(r"(.+?)_stage1_", path.name)
        if match:
...
def collect_input_paths(input_dir: Path | None, inputs: list[str] | None, run_id: str | None) -> list[Path]:
    ...
    pattern = f"{run_id}_stage1_*_review.json" if run_id else "*_stage1_*_review.json"
    return [
        path
        for path in sorted(input_dir.glob(pattern))
        if not path.name.endswith("_stage1_review.json")
    ]
...
def function_id(data: Any, path: Path) -> str:
    if isinstance(data, dict) and isinstance(data.get("meta"), dict):
        ...
        if fid:
            return fid
    match = re.search(r"_stage1_([^_]+)_review\.json$", path.name)
    if match:
        return match.group(1)
...
def merge_reviews(paths: list[Path], run_id: str, stage0_data: Any | None = None) -> dict[str, Any]:
    ...
    for path in paths:
        ...
        if fid in seen_functions:
            raise SystemExit(f"Duplicate Stage1R review slice for Function_ID {fid}: {path}")
        seen_functions.add(fid)
        source_files.append(str(path))

        result = str(data.get("overall_result") or "").strip().lower()
        if result:
    ...
    if expected_ids:
        ...
        if missing:
            raise SystemExit(f"Missing Stage1R review slices for Function_ID: {', '.join(missing)}")
        if extra:
            raise SystemExit(f"Stage1R review slices reference unknown Function_ID: {', '.join(extra)}")
    ...
    return {
        "meta": {
            "run_id": run_id,
            "stage": "stage1_review",
            "target_file": f"output/{run_id}_stage1_derive_mf.json",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Merge Stage1R single-function review JSON files.")
    parser.add_argument("--inputs", nargs="*", help="Explicit Stage1R review slice files")
    parser.add_argument("--input-dir", help="Directory containing <RUN_ID>_stage1_<Function_ID>_review.json files")
    ...
    parser.add_argument("--out", required=True, help="Merged Stage1R review output path")
    args = parser.parse_args()

    input_dir = Path(args.input_dir) if args.input_dir else None
    paths = collect_input_paths(input_dir, args.inputs, args.prefix)
    if not paths:
        raise SystemExit("No Stage1R review slice files found.")
